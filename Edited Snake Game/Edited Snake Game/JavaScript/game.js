import { mySel, numOfRow, numOfBlocks, mySelAll } from "./constants.js";
...
function gameLoop() {
  changeMove = setInterval(() => {
    ...
    if (
      move == "ArrowRight" &&
      lastMoves[lastMoves.length - 2] != "ArrowLeft"
    ) {
      right(col);
    }
    ...
    if (
      move == "ArrowLeft" &&
      lastMoves[lastMoves.length - 2] != "ArrowRight"
    ) {
      left(col);
    }
...
function changeDirection() {
  ...
  snake.forEach((cur, index) => {
    ...
    mySel("table").children[0].children[cur[0]].children[cur[1]].classList.add(
      "active"
    );
  });
}
